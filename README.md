...
`New-ModDossier.ps1` answers the question everybody actually asks — *"I installed this, is it working?"* — for one mod, across every layer it ships. A mod isn't one thing: it's up to nine payloads in nine places, and each fails silently in its own way. So it reports per layer, not as a single verdict: where the archive sits in the load order, whether the `.reds` is in the compiled bundle, whether the CET folder has an `init.lua`, plus your own settings for that mod and any override you've registered against it.

`New-ArchiveAnatomy.ps1` describes the load order as a thing in itself, which nothing else here does: how much of the base game it replaces versus how much it invents, broken down by area and by file type, plus which archives reach deepest into vanilla and which have lost every file they ship. The replace-versus-add split is only possible because of the vendored path table below — a hash the table knows is an override of a file the game shipped, and a hash it does not know is an asset the mod authored. Every tool that reads archives without that table sees the two as identical.

`New-ModCredits.ps1` is the one report here that is not a diagnostic: every author whose work is in your game and what each of them made, as end credits. Adult mods are omitted by default and the omitted count is printed, because the page is built to be shown to other people. **Every HTML report takes `-Md`** and writes the same facts as markdown, for a forum post or a Discord message — with the redaction intact, and a warning when it runs past the 2000 characters Discord refuses rather than truncates.
...
- This was created with **game patch 2.31**. Paths and behaviours drift between patches, and they drift at very different rates. I'll make an effort to update this if CDPR updates the game, but until I do, you should assume Cyberwise is broken.
...
**One file is not mine and not MIT.** `skills/cyberwise-conflicts/data/resource-paths-2.31.cwpx` is derived from [VanStorm's resource-path database](https://github.com/VanStorm/Cyberpunk-Modding) and stays under **CC BY 4.0** — free to use and redistribute, including commercially, as long as VanStorm is credited. If you fork this repo, that credit travels with the file; `data/ATTRIBUTION.md` is written to be exactly what you need to keep.
